...
MpfrClass::MpfrClass(const mpfr_t& mpfr,
		      RoundingMode rnd,
                    PrecisionType prec)
  {
  mpfr_init2(mpfr_rep, prec);
    
  mpfr_init_set(mpfr_rep, mpfr, GMP_RNDN);
...
//--------------------------------------------------------------
//
mpfr_t& MpfrClass::getValue(void)
  {
...
MpfrClass pow (const MpfrClass& r,
                const int e, 
                RoundingMode rnd)
  {
  MpfrClass res;
  ...
  res.nbref->refvalue() = 1;
  res.inexact->refvalue() = mpfr_pow_si(res.mpfr_rep, r.mpfr_rep, e, rnd);
  return res;
  }
...
MpfrClass pow (const MpfrClass& r, 
  ...
  res.nbref->refvalue() = 1;
  res.inexact->refvalue() = mpfr_pow_ui(res.mpfr_rep, r.mpfr_rep, e, rnd);
  return res;
  }
...
MpfrClass pow (const MpfrClass& r, 
  ...
  mpfr_init_set_d(e_as_mpfr, e, GMP_RNDN);
  
  res.nbref->refvalue() = 1;
  res.inexact->refvalue() = mpfr_pow(res.mpfr_rep, r.mpfr_rep, e_as_mpfr, rnd);
  mpfr_clear(e_as_mpfr);
  return res;
  }
...
MpfrClass pow (const MpfrClass& r,
  ...
  res.nbref->refvalue() = 1; 
  res.inexact->refvalue() = mpfr_pow(res.mpfr_rep, r.mpfr_rep, e.mpfr_rep, rnd);
  return res;
  }

/*void MpfrClass::pow (const int e, RoundingMode rnd)
  {
  this->inexact->refvalue() = 
              mpfr_pow_si(this->mpfr_rep, this->mpfr_rep, e, rnd);
  }

void MpfrClass::pow (const unsigned long int e, RoundingMode rnd)
  {

  this->inexact->refvalue() = 
              mpfr_pow_ui(this->mpfr_rep, this->mpfr_rep, e, rnd);
  }

void MpfrClass::pow (const long int e, RoundingMode rnd)
  {
  this->inexact->refvalue() = 
              mpfr_pow_si(this->mpfr_rep, this->mpfr_rep, e, rnd);
  }

void MpfrClass::pow (const double pow, RoundingMode rnd)
  {
  mpfr_t pow_as_mpfr;
  
  mpfr_init_set_d(pow_as_mpfr, pow, GMP_RNDN);
  
  this->inexact->refvalue() = 
              mpfr_pow(this->mpfr_rep, this->mpfr_rep, pow_as_mpfr, rnd);
  mpfr_clear(pow_as_mpfr);
  }
  

void MpfrClass::pow (const MpfrClass& e, RoundingMode rnd)
  {
  this->inexact->refvalue() = 
              mpfr_pow(this->mpfr_rep, this->mpfr_rep, e.mpfr_rep, rnd);
  }
